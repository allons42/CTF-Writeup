...
class Market():
    def __init__(self, flag):
        ...
        self.user = False
        self.NL = {b"admin":self.p*self.q}
        self.balance = defaultdict(int)
        self.store = [Flag(b'admin', 0x233333333333333, flag), Flag(b'admin', 0x10, "fakeflag")]
        self.command = [b"register", b"login", b"view", b"sell", b"buy", b"setprice", b"logout", b"exit"]
        self.USER_MAX = 10
        self.ITEM_MAX = 100
        print(f"\033[1;33;31mwelcome to supper safe flagmarket\033[0m\n\033[1;32mBase:\033[0m \033[1;33m{self.NL[b'admin']}\033[0m\n")
    ...
    def register(self):
        ...
        if (len(self.NL) < self.USER_MAX) and (u not in self.NL):
            self.NL[u] = self.NL[b'admin'] + random.randint(2**32, 2**64)
            self.balance[u] = 100
            self.user = u
    ...
    def view(self, idx, sig):
        ...
        item = self.store[idx]
        if item.getOwner() == self.user:
            if idx > 0:
                if self.verify(self.user, self.command[2], sig):
                    print(item.content)
            else:
                if self.verify(self.user, b"getflag", sig):
                    print(item.content)
    ...
    def sign(self, u, message):
        ...
        return long_to_bytes((alpha*sig_p+beta*sig_q) % self.NL[u]).hex()

    def verify(self, user, message, sig):
        if not self.user:
            return
        return long_to_bytes(pow(bytes_to_long(sig), self.e, self.NL[user])) == sha256(message).digest()
